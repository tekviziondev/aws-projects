"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Wait = void 0;
const contact_flow_processor_1 = require("../contact-flow-processor");
const call_details_1 = require("../utility/call-details");
const chime_action_types_1 = require("../const/chime-action-types");
const constant_values_1 = require("../const/constant-values");
const default_termination_action_1 = require("../utility/default-termination-action");
const constant_values_2 = require("../const/constant-values");
/**
  * Making a SMA action to perform Wait for a specified period of time.
  * @param smaEvent
  * @param action
  * @param actions
  * @param amazonConnectInstanceID
  * @param bucketName
  * @param contextStore
  * @returns SMA action
  */
class Wait {
    async processFlowActionWait(smaEvent, action, actions, amazonConnectInstanceID, bucketName, contextStore) {
        let callId;
        try {
            // getting the CallID of the Active call from the SMA Event
            let callDetails = new call_details_1.CallDetailsUtil();
            const legA = callDetails.getLegACallDetails(smaEvent);
            callId = legA.CallId;
            if (!callId)
                callId = smaEvent.ActionData.Parameters.CallId;
            console.log(constant_values_2.Attributes.DEFAULT_LOGGER + callId + " Pause Action");
            let timeLimit = getWaitTimeParameter(action);
            let smaAction = {
                Type: chime_action_types_1.ChimeActions.PAUSE,
                Parameters: {
                    "DurationInMilliseconds": timeLimit //Mandatory
                }
            };
            const nextAction = callDetails.findActionObjectByID(actions, action.Transitions.Conditions[0].NextAction);
            console.log(constant_values_2.Attributes.DEFAULT_LOGGER + callId + " Next Action identifier:" + nextAction);
            console.log(constant_values_2.Attributes.DEFAULT_LOGGER + callId + " Pause action is Performed for " + timeLimit + " Milliseconds");
            contextStore[constant_values_1.ContextStore.PAUSE_ACTION] = smaAction;
            return await (0, contact_flow_processor_1.processFlowAction)(smaEvent, nextAction, actions, amazonConnectInstanceID, bucketName, contextStore);
        }
        catch (error) {
            console.error(constant_values_2.Attributes.DEFAULT_LOGGER + callId + " There is an error in execution of Wait action " + error.message);
            return await new default_termination_action_1.TerminatingFlowUtil().terminatingFlowAction(smaEvent, "error");
        }
    }
}
exports.Wait = Wait;
function getWaitTimeParameter(action) {
    let rv;
    if (action.TimeLimitSeconds !== null) {
        const timeLimitSeconds = Number.parseInt(action.Parameters.TimeLimitSeconds);
        rv = String(timeLimitSeconds * 1000);
    }
    console.log("Wait Parameter : " + rv);
    return rv;
}
